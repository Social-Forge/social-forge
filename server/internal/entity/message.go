...
type Message struct {
	...
	Content          NullString       `json:"content,omitempty" db:"content"`
	MediaURL         NullString       `json:"media_url,omitempty" db:"media_url"`
	MediaType        NullString       `json:"media_type,omitempty" db:"media_type"`
	MediaSize        NullInt64        `json:"media_size,omitempty" db:"media_size"`
	ThumbnailURL     NullString       `json:"thumbnail_url,omitempty" db:"thumbnail_url"`
	ChannelMessageID NullString       `json:"channel_message_id,omitempty" db:"channel_message_id"`
	...
	SearchVector     NullString       `json:"-" db:"search_vector"`
...
func (m *Message) MarkAsDelivered() {
	...
	m.DeliveredAt = NewNullTime(now)
}

func (m *Message) MarkAsRead() {
	now := time.Now()
	m.Status = MessageStatusRead
	m.ReadAt = NewNullTime(now)
}

func (m *Message) MarkAsFailed(errorMsg string) {
	now := time.Now()
	m.Status = MessageStatusFailed
	m.FailedAt = NewNullTime(now)
	m.ErrorMessage = NewNullString(errorMsg)
}
