...
CREATE INDEX IF NOT EXISTS idx_sessions_access_token ON sessions (access_token) WHERE is_revoked = FALSE AND expires_at > now();
CREATE INDEX IF NOT EXISTS idx_sessions_refresh_token ON sessions (refresh_token) WHERE is_revoked = FALSE AND expires_at > now();
