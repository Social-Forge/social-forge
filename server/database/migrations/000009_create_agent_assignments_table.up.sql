CREATE TABLE agent_assignments (
  ...
  status VARCHAR(255) NOT NULL DEFAULT 'available' CHECK (status IN ('available', 'busy', 'offline')),
...
CREATE INDEX idx_agent_assignments_tenant_id ON agent_assignments(tenant_id);
CREATE INDEX idx_agent_assignments_division_id ON agent_assignments(division_id);
CREATE INDEX idx_agent_assignments_user_id ON agent_assignments(user_id);
CREATE INDEX idx_agent_assignments_is_active ON agent_assignments(is_active);
CREATE INDEX idx_agent_assignments_status ON agent_assignments(status);
CREATE INDEX idx_agent_assignments_created_at ON agent_assignments(created_at);
